...
def maliciousProgram(curDate, changes):
    ...
    date=datetime.strptime(curDate,dateFormat)
    dateList=[date.day,date.month,date.year,date.hour,date.minute,date.second]
    newDate=[x+y for x,y in zip(dateList,changes)]
    try: 
        Date=datetime(
            day=newDate[0],
            month=newDate[1],
